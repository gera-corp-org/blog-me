...
**Файлы:**
- Создать: `src/domain/format.js`, `src/routes/public/posts.js`,
...
- Тесты: `test/helpers/app.js`, `test/public.test.js`
...
```js
  app.get('/tag/:slug', async (request, reply) => {
    ...
    const perPage = app.config.postsPerPage;
    const requested = Number.parseInt(request.query.page ?? '1', 10);
    const total = app.posts.countByTag(tag.slug);
    const pages = Math.max(1, Math.ceil(total / perPage));
    const page = Math.min(Math.max(Number.isFinite(requested) ? requested : 1, 1), pages);
    ...
    return reply.view('tag.eta', {
      ...
      tag,
      posts: app.posts.listByTag(tag.slug, { limit: perPage, offset: (page - 1) * perPage }),
      page,
      pages,
    });
  });
