...
export function loadConfig(env = process.env) {
  ...
  return {
    ...
    dataDir,
    // Пути от dataDir склеиваются строкой, а не join: join нормализует
    // «./data» в «data», а вид пути важен — он попадает в логи и в README.
...
- Создать: `migrations/002_search.sql`, `src/domain/search.js`,
  ...
  `views/partials/post-card.eta`
- Изменить: `src/db/posts.js`, `src/server.js`, `views/index.eta`,
  `views/tag.eta`
...
Карточка записи — третья по счёту копия одной и той же разметки (лента,
тег, поиск). По правилу трёх выносим её в общий кусок; ревью задачи 8
отложило это решение ровно до третьего случая.

`views/partials/post-card.eta`:

```html
<article class="post-card">
  <h2><a href="/p/<%= it.post.slug %>"><%= it.post.title %></a></h2>
  <p class="meta"><time datetime="<%= it.post.published_at %>"><%= it.formatDate(it.post.published_at) %></time></p>
  <p><%= it.post.excerpt %></p>
...
Кусок получает свой набор данных, поэтому `formatDate` передаётся ему явно.

В `views/index.eta` заменить блок `<article class="post-card">…</article>`
на вызов с тегами:

```html
<% it.posts.forEach(function (post) { %>
  <%~ include('/partials/post-card', { post, tags: post.tags, formatDate: it.formatDate }) %>
<% }) %>
```

В `views/tag.eta` — на такой же вызов, но без тегов: на странице тега
незачем повторять ссылкой тот самый тег, который читатель уже открыл.

```html
<% it.posts.forEach(function (post) { %>
  <%~ include('/partials/post-card', { post, formatDate: it.formatDate }) %>
<% }) %>
```
...
- [ ] **Шаг 4: Поставить сторожа против невидимых символов в исходниках**

Диапазоны управляющих символов пишутся escape-последовательностями. Живые
символы в исходнике невидимы в редакторе, не находятся поиском и однажды
уже привели ревьюера к неверному выводу о коде.
...
data:
  ...
  SITE_AUTHOR: "Гера"
  # Ingress стоит в частной сети кластера, поэтому его заголовку с адресом
  # посетителя можно верить. Без этого адресом каждого посетителя окажется
  # адрес самого ingress, и ограничение попыток входа станет общим на всех.
...
Ожидается: манифесты собираются, на выходе шесть ресурсов. Это
единственная проверка, работающая без кластера: `--dry-run=client`
вопреки названию требует доступа к серверу — он скачивает описание схемы
и список типов ресурсов, а без них не может даже опознать `Deployment`.
Полная проверка делается при подключённом кластере:
...
## Что изменилось после финального ревью ветки

Ревью всей ветки целиком нашло девять пунктов, которых не было видно
внутри отдельных задач. Все закрыты одной волной правок (коммиты
`0a517e5`..`9f2baa4`); текст задач выше их не описывает, потому что
изменения затрагивают сразу несколько задач.

1. **Подтверждение удаления записи не работало.** Встроенный обработчик
   `onsubmit` блокируется политикой безопасности, которую приложение само
   ставит на каждый ответ. Подтверждение вынесено в `public/admin.js`
   через `addEventListener`, сторож встроенных скриптов теперь заходит и
   на страницы админки.
2. **Ключ защиты форм подписан** той же подписью, что и ключ сессии:
   раньше вся защита сводилась к настройке `SameSite`.
3. **Ключ не выдаётся** на статику, картинки и пробы кластера — их ответы
   кешируются надолго, и общий кеш вправе раздать один ключ всем.
4. **`verifyCsrf` отвечает JSON** запросам из редактора и страницей —
   человеку с формой.
5. **Просроченные сессии** чистятся не только при старте, но и на том же
   суточном тике, что и снимок базы.
6. **Теги на карточке записи** выводятся и на странице поиска, как в
   ленте. На странице тега их по-прежнему нет намеренно.
7. **Тесты**: отказ по ключу на записывающих маршрутах, честное сравнение
   предпросмотра с сохранённым HTML, экранирование в шаблонах, свойства
   сессионной cookie, детерминированный порядок ленты.
8. **`.dockerignore`** пополнен служебными каталогами.
9. **README**: секрет создаётся после применения манифестов (раньше
   команда падала бы на несуществующем пространстве имён), добавлено
   пояснение про доступность образа узлам кластера.

Известный долг, не закрытый сознательно: запись в лог при сбое суточного
расписания не различает падение снимка и падение уборки сессий.
